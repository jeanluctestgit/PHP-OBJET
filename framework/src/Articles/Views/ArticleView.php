...
<div class="card mb-3">
    ...
    <form action="<?= isset($_POST['titre']) ? '/articles/update/'. $_POST['id'] : '/articles/add' ?>" method="post">
    <div class="row">
    ...
    <?php
      ...
      if(!empty($articles)){
        ...
        foreach ($articles as $key => $value) {
            ...
            echo '<span class= "float-right ml-3" >' . '<form action="/articles" method="post">' 
            ...
            echo '<span class="float-right">' . '<form action="/articles/delete/' . $value['id'] . '" method="post">' . '<input type="hidden" class="form-control" name="id" id="id" value = "' . $value['id'] .'" >' . '<button type="submit" class="btn btn-danger ">X</button>' .'</form>' . '</span>';
